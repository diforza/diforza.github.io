(function () {
    ...
    /**********************************************************
     * InfoBlock: заголовок, детали, фон, PG
     **********************************************************/
    function InfoBlock() {
        let html, timer;
        let loaded = {};
        ...
        this.draw = function (data) {
            ...
            let pg = (typeof window.getInternationalPG === 'function')
                ? window.getInternationalPG(data)
                : Lampa.Api.sources.tmdb.parsePG(data);

            let blocks = [];
            if (year || countries.length) {
                blocks.push(`<div class="new-interface-info__block">${
                    [year, countries.join(', ')].filter(Boolean).join(', ')
                }</div>`);
            }
            if (genres.length) {
                blocks.push(`<div class="new-interface-info__block">${genres.join(' | ')}</div>`);
            }
            if (pg) {
                blocks.push(`<div class="new-interface-info__block new-interface-info__pg">${pg}</div>`);
            }
            ...
            onInstance(line) {
                ...
                line.use({
                    onInstance(card, card_data) {
                        card.use({
                            onEnter() { Lampa.Router.call('full', card_data); }
                        });
                    }
                });
            }
    ...
    Lampa.Template.add('new_interface_style_v3_full', `
        <style>
        .new-interface-info { padding: 0 1.5em; }
        .new-interface-info__body { width: 95%; padding-top: 1.1em; }
        .new-interface-info__title {
            font-size: 4em; font-weight: 600; margin-bottom: 0.5em; line-height: 1;
            overflow: hidden; display: -webkit-box; -webkit-line-clamp: 1; -webkit-box-orient: vertical;
        }
        .new-interface-info__details {
            display: flex; flex-wrap: wrap; align-items: center; gap: 0; font-size: 1.2em;
        }
        .new-interface-info__block {
            border: 1px solid rgba(255,255,255,1); padding: 0.3em 0.5em; white-space: nowrap;
        }
        .new-interface-info__separator { font-size: 1.5em; font-weight: 900; margin: 0 0.2em; opacity: 0.8; }
    ...
    $('body').append(Lampa.Template.get('new_interface_style_v3_full', {}, true));
