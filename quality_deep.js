(function() {
    ...
    var LQE_CONFIG = {
        ...
        MANUAL_OVERRIDES: {
            ...
            '392': { quality_code: 1080, full_label: '1080p BDRemux' },
            '7985': { quality_code: 1080, full_label: '1080p BDRemux' },
            '11141': { quality_code: 2160, full_label: '1080p HDTVRip' }
            //'20977': { quality_code: 1080, full_label: 'HDTVRip-AVC' }
        }
    };
    ...
    var QUALITY_DISPLAY_MAP = {
        ...
        "WEBRip [H.265/2160p] [звук с TS] [AD]": "4K WEBRip/Ts",
    ...
    function isAllowedSource(cardData) {
        // Проверяем несколько возможных мест, где может быть source
        var source = cardData.source || cardData.card_source || '';
        
        // Если source не найден в cardData, проверяем родительские элементы
        if (!source && cardData.card) {
            var cardElement = cardData.card;
            if (cardElement.getAttribute) {
                source = cardElement.getAttribute('data-source') || '';
            }
        }
        
        var allowedSources = ['tmdb', 'tmdb_mod', 'cub', 'SURS', 'SURS RUS', 'SURS KIDS', 'SURS NEW'];
        
        // Если source пустой или в списке разрешенных - разрешаем
    ...
    function updateCardListQuality(cardElement) {
        ...
        if (cardData) {
        console.log('--- LQE DEBUG ---');
        console.log('Название:', cardData.title || cardData.name);
        console.log('Источник (source):', cardData.source);
        console.log('Год:', cardData.release_date || cardData.first_air_date || 'Не указан');
        console.log('-----------------');
    }
        ...
        // Проверяем разрешен ли источник - для главной страницы source может быть пустым
        if (!isAllowedSource(cardData)) {
            if (LQE_CONFIG.LOGGING_CARDLIST) console.log("LQE-CARDLIST", "Skipping card from unsupported source:", cardData.source, "Card:", cardData.id);
            return;
        }
        ...
        if (!cardId) {
            if (LQE_CONFIG.LOGGING_CARDLIST) console.log("LQE-CARDLIST", "Card ID is empty, skipping:", normalizedCard);
            return;
        }
    ...
    function processAllCards() {
        ...
        if (allCards.length) {
            if (LQE_CONFIG.LOGGING_GENERAL) console.log('LQE-LOG: Processing all existing cards:', allCards.length);
    ...
    function initializeLampaQualityPlugin() {
        ...
        // Обрабатываем существующие карточки сразу
        setTimeout(processAllCards, 100);
        
        // Периодическая проверка на появление новых карточек (на случай если MutationObserver пропустил)
